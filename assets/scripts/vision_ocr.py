...
This script covers the optional local-inference tiers. The default `grade`
path uses the opencode agent's native vision instead (no external model); this
script is invoked by the harness only when the course's `OCR_ENGINE=ollama` or
...
Usage:
    python vision_ocr.py <input.pdf> <output.md>
    python vision_ocr.py --engine=ollama    <input.pdf> <output.md>
    python vision_ocr.py --engine=tesseract <input.pdf> <output.md>
    python vision_ocr.py --engine=ollama \\
...
def ocr_pdf(
    ...
    # generic default. Works both when invoked by the `grade` command (CWD =
    # course root) and when run standalone.
    effective_course = (
        course_name
...
def _parse_args(argv: list[str]) -> tuple[str, Path, Path, str | None, str | None]:
    ...
    if len(positional) != 2:
        print(
            "usage: python vision_ocr.py [--engine=ollama|tesseract] "
